namespace SXF.Kernel
{
    ...
    public class PagerManager
    {
        ...
        public PagerSet GetPagerSet2(PagerParameters pramsPager)
        {
            ...
            //EventLog.WriteLog(pramsPager.Table);
            //EventLog.WriteLog(this.GetFieldString(pramsPager.Fields, pramsPager.FieldAlias));
            //EventLog.WriteLog(pramsPager.WhereStr);
            //EventLog.WriteLog(pramsPager.PKey);
        ...
        /// <param name="pramsPager"></param>
        /// <returns></returns>
        public PagerSet GetPagerSet3(PagerParameters pramsPager)
        {
            DataSet set;
            if (this.m_prams == null)
            {
            ...
            //EventLog.WriteLog(this.GetFieldString(pramsPager.Fields, pramsPager.FieldAlias));
           // EventLog.WriteLog(pramsPager.PageIndex.ToString());
            //EventLog.WriteLog(pramsPager.WhereStr);
            this.m_dbHelper.RunProc("WEB_PageView3", prams, out set);
            //EventLog.WriteLog("盛学飞");
            return new PagerSet(pramsPager.PageIndex, pramsPager.PageSize, Convert.ToInt32(prams[prams.Count - 3].Value), Convert.ToInt32(prams[prams.Count - 2].Value), set) { PageSet = { DataSetName = "PagerSet_" + pramsPager.Table } };
