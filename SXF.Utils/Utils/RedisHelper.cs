...
namespace SXF.Utils
{
    ...
    {
        private static string _conn = ConfigurationManager.AppSettings["redis_connection_string"] ?? "127.0.0.1:8383";
        private static string _pwd = ConfigurationManager.AppSettings["redis_connection_pwd"] ?? "017fe5e8-ad51-4fb3-b96a-c21d6f67a3e3";
        ...
        /// <param name="db">库，默认读取配置文件</param>
        /// <returns></returns>
        public static async Task<List<T>> ListRangeAsync<T>(string key, long start = 0, long stop = -1, RedisFolderEnum folder = RedisFolderEnum.Root, RedisDBEnum db = RedisDBEnum.Default)
        {
            string fd = GetDescription(folder);
            var context = Manager.GetDatabase(GetOperationDB(db));
            var value = await context.ListRangeAsync(string.IsNullOrEmpty(fd) ? key : fd + ":" + key, start, stop);
            //EventLog.WriteLog("" + (value == null));
            return ConvetList<T>(value);
        }
        ...
        /// <param name="db">库，默认读取配置文件</param>
        /// <returns></returns>
        public static async Task<long> ListRightPushAsync<T>(string key, List<T> values, RedisFolderEnum folder = RedisFolderEnum.Root, RedisDBEnum db = RedisDBEnum.Default)
        {
            string fd = GetDescription(folder);
            var redisValues = values.Select(m => (RedisValue)ConvertJson(m)).ToArray();
            return await Manager.GetDatabase(GetOperationDB(db)).ListRightPushAsync(string.IsNullOrEmpty(fd) ? key : fd + ":" + key, redisValues);
        }
